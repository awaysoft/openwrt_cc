...
// liudf added 20150331

static int mt7620_wx_get_range(struct net_device *dev,
                            ...
                            union iwreq_data *wrqu, char *extra)
{
	...
	range->num_channels =  ((PRTMP_ADAPTER)pAd)->ChannelListNum;
	//printk("num_channels is %d===\n", range->num_channels);	
	os_alloc_mem(NULL, (UCHAR **)&pChannel, sizeof(UCHAR)*range->num_channels);
        if (pChannel == NULL)           
                return -ENOMEM;
        os_alloc_mem(NULL, (UCHAR **)&pFreq, sizeof(UINT32)*range->num_channels);
        if (pFreq == NULL)      
        {
	...
	RTMP_COM_IoctlHandle(pAd, NULL, CMD_RTPRIV_IOCTL_CHAN_LIST_GET, 0,
                             pChannel, 0);
        RTMP_COM_IoctlHandle(pAd, NULL, CMD_RTPRIV_IOCTL_FREQ_LIST_GET, 0,
                             pFreq, 0);
...
static int mt7620_wx_set_scan(struct net_device *dev,
                           struct iw_request_info *info,
                           union iwreq_data *wrqu, char *extra)
{
	VOID *pAd = NULL;
...
static int mt7620_wx_get_scan(struct net_device *dev,
                           struct iw_request_info *info,
                           union iwreq_data *wrqu, char *extra)
{
	RTMP_ADAPTER                    *pAd;
...
static iw_handler mt7620_wx_handlers[] = {
	IW_HANDLER(SIOCGIWRANGE, mt7620_wx_get_range),
	IW_HANDLER(SIOCSIWSCAN, mt7620_wx_set_scan),
        IW_HANDLER(SIOCGIWSCAN, mt7620_wx_get_scan),
};
// << end liudf added
...
#define	N(a)	(sizeof (a) / sizeof (a[0]))
	// liudf added 20150331
	.standard	= mt7620_wx_handlers,
	.num_standard 	= N(mt7620_wx_handlers),
	// << end
...
/*				PCHAR pBssidStr; */
		...
		case SIOCGIWSCAN:
			...
			printk("SIOCGIWSCAN result is %d === \n", 
				((PRTMP_ADAPTER)pAd)->ScanTab.BssNr);
			Status = RTMP_IO_EOPNOTSUPP;
#endif
			break;
		case SIOCGIWRANGE:	/*Get range of parameters */
