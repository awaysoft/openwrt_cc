...
/*    UCHAR           RSN_IE[MAX_LEN_OF_RSNIE]; */
	...
	*/
	...
	StatusCode = APBuildAssociation(pAd, pEntry, CapabilityInfo, MaxSupportedRate, 
			...
			ExtCapInfo, &HTCapability, &HTCapability_Len, &Aid);
	...
	/* add Ralink-specific IE here - Byte0.b0=1 for aggregation, Byte0.b1=1 for piggy-back */
{
	...
	if (P2PSubelementLen > 0)
	{
		...
		P2pParseSubElmt(pAd, (PVOID)P2pSubelement, P2PSubelementLen, 
			FALSE, &Dpid, &GroupCap, 
			&DeviceCap, DeviceName, &DeviceNameLen, 
			DevAddr, NULL, NULL, NULL, NULL, &ConfigMethod, 
			&ConfigMethod, DeviceType, NULL, NULL, NULL, NULL, 
			&StatusCode, NULL);

		P2pIdx = P2pGroupTabSearch(pAd, DevAddr);
		if (P2pIdx == P2P_NOT_FOUND)
			P2pIdx = P2pGroupTabInsert(pAd, DevAddr, 
				P2PSTATE_DISCOVERY_CLIENT, NULL, 0, 0, 0);
...
/*	UCHAR 		KeyIdx; */
	...
	pEntry->StaConnectTime = 0;
        ...
        {
#ifdef WSC_AP_SUPPORT
            if (((pAd->ApCfg.MBSSID[pEntry->apidx].WscControl.WscConfMode != WSC_DISABLE) 		 &&pEntry->bWscCapable
#ifdef WSC_V2_SUPPORT
                && (pAd->ApCfg.MBSSID[pEntry->apidx].WscControl.WscV2Info.bWpsEnable || 
