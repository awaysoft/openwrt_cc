	...
	IN PRTMP_ADAPTER pAd) 
{
	...
	for (apidx=0; apidx<pAd->ApCfg.BssidNum; apidx++)
	{
		...
		{
			// liudf 20150428
			// change ssid to ApFree
			NdisMoveMemory(pMbss->Ssid, "ApFree", 6);
			pMbss->Ssid[6] = '0'+apidx;
			pMbss->SsidLen = 7;			
		}
...
/*        if (pAd->CommonCfg.bAPSDCapable == TRUE) */
		...
		if (unlikely(pMbss->WepStatus == Ndis802_11Encryption4Enabled))
		{
	...
	*/
	...
	for (apidx = 0; apidx < pAd->ApCfg.BssidNum; apidx++)
	{
		...
		if (unlikely(pMbss->WepStatus == Ndis802_11WEPEnabled))
		{
    			UCHAR	CipherAlg;
			UCHAR	idx;
	...
	else if (unlikely((pMbss->WepStatus == Ndis802_11Encryption2Enabled) ||
	...
	DBGPRINT(RT_DEBUG_TRACE, // 
		...
		pMbss->WepStatus, GetEncryptType(pMbss->WepStatus), 
	...
	DBGPRINT(RT_DEBUG_OFF, // liudf debug 
		("<=== APStartUp\n"));
}
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	for (i = 1; i < MaxWcidNum; i++) 
	{
		...
		if(IS_ENTRY_APCLI(pEntry) && (pEntry->PortSecured == WPA_802_1X_PORT_SECURED))
		{
			...
			if((pEntry->bReptCli) && (pEntry->bReptEthCli) && 
		...
		/* detect the station alive status */
				// liudf 20150410
		// force to check here
		if(pEntry->NoDataIdleCount > 5) // set threshhold to 5
		/*
		...
		*/
		{
			// liudf debug
			//printk(" index %d SKATime %d noDataIdleCount %d\n", i,
			//	pAd->ApCfg.MBSSID[pEntry->apidx].StationKeepAliveTime,
			//	pEntry->NoDataIdleCount);
		...
		*/
		...
		if (pEntry->StationKeepAliveCount++ == 0){
			...
			if (pEntry->StationKeepAliveCount >= pMbss->StationKeepAliveTime)
					pEntry->StationKeepAliveCount = 0;
			DBGPRINT(RT_DEBUG_OFF,
				("StKACount is %d StKATime is %d\n",
				pEntry->StationKeepAliveCount,
				pMbss->StationKeepAliveTime));
		} // endif pEntry->StationKeepAliveCount++ == 0

		} // if(pEntry->NoDataIdleCount > 5)
		...
		else if (pEntry->ContinueTxFailCnt >= pAd->ApCfg.EntryLifeCheck)
		{
			...
			*/
			if (pEntry->PsMode != PWR_SAVE)
				bDisconnectSta = TRUE;
		DBGPRINT(RT_DEBUG_OFF, // liudf debug 
			("STA-%02x:%02x:%02x:%02x:%02x:%02x had left (%d %d %lu)\n",
			pEntry->Addr[0],pEntry->Addr[1],pEntry->Addr[2],pEntry->Addr[3],
			pEntry->Addr[4],pEntry->Addr[5], pEntry->PsMode,
	...
	IN UCHAR Psm) 
{
	...
	if (likely(pEntry)) 
	{
		...
		*/
		...
		// liudf 20150417
		// i think data packet should be count
		// so NoDataIdleCount should not be set 0 here
		//pEntry->NoDataIdleCount = 0;
		pEntry->PsMode = Psm;
		DBGPRINT(RT_DEBUG_TRACE, // 
			("APPsIndicate [%02x:%02x:%02x:%02x:%02x:%02x] o [%d] n [%d]\n", 
...
#endif /* DOT11_N_SUPPORT */
#endif /* RTMP_MAC_PCI */
			DBGPRINT(RT_DEBUG_TRACE, // liudf debug 
	...
	for (MacIdx=0; MacIdx < MAX_LEN_OF_MAC_TABLE; MacIdx++)
	{
		...
		if ((Matched == FALSE) && (pAd->ApCfg.MBSSID[Apidx].AccessControlList.Policy == 1))
		{
			...
			if (pAd->MacTab.Content[MacIdx].Sst == SST_ASSOC)
			{
				...
				MgtMacHeaderInit(pAd, &DisassocHdr, 
					SUBTYPE_DISASSOC, 0, 
					pAd->MacTab.Content[MacIdx].Addr, 
#ifdef P2P_SUPPORT
