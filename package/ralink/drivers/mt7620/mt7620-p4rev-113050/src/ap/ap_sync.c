	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	if ( pAd->P2pCfg.bSentProbeRSP != TRUE )
		return;
#endif /* P2P_SUPPORT */
	if (! PeerProbeReqSanity(pAd, Elem->Msg, Elem->MsgLen, 
		Addr2, Ssid, &SsidLen, &bRequestRssi)) {
		return;
	}
	...
	BND_STRG_CHECK_CONNECTION_REQ(	pAd,
		...
		MgtMacHeaderInit(pAd, &ProbeRspHdr, SUBTYPE_PROBE_RSP, 0, Addr2, 
		 ...
		 if ((pAd->ApCfg.MBSSID[apidx].AuthMode == Ndis802_11AuthModeWPA) ||
			...
			RSNIe = IE_WPA2;
#ifdef WAPI_SUPPORT
		else if ((pAd->ApCfg.MBSSID[apidx].AuthMode == Ndis802_11AuthModeWAICERT) 			||(pAd->ApCfg.MBSSID[apidx].AuthMode == Ndis802_11AuthModeWAIPSK))
			RSNIe = IE_WAPI;
#endif /* WAPI_SUPPORT */
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (MlmeScanReqSanity(pAd, Elem->Msg, Elem->MsgLen, &BssType, 
		(PCHAR)Ssid, &SsidLen, &ScanType))
	{
#ifdef DOT11_N_SUPPORT
	...
	IN	BOOLEAN				ChannelSel)
{
	...
	if (pSsid)
	{
    	...
    	bret = MlmeEnqueue(pAd, AP_SYNC_STATE_MACHINE, APMT2_MLME_SCAN_REQ, 
