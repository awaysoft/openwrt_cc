...
{ 
	...
	if (RTMPEqualMemory(pMacEntry->Addr, pAddr, MAC_ADDR_LEN))
	{
		...
		*/
		...
		("BEGIN: rcv PS-POLL (AID=%d) from %02x:%02x:%02x:%02x:%02x:%02x\n", 
				...
				*/
					...
					if (CLIENT_STATUS_TEST_FLAG(pMacEntry, fCLIENT_STATUS_WMM_CAPABLE))
						bQosNull = TRUE;
	            			...
	            			ApEnqueueNullFrame(pAd, pMacEntry->Addr, 
						pMacEntry->CurrTxRate,
	...
	IN RX_BLK *pRxBlk)
{
	...
	do
	{
		...
		if (unlikely((pHeader->FC.SubType == SUBTYPE_AUTH) && 
	...
	IN	RX_BLK		*pRxBlk)
{
	...
	*/
	...
	// liudf 20150417
	// set NoDataIdleCount 0 here;
	//printk("APHandleRxDataFrame set NoDataIdleCount 0 here ===>\n");
