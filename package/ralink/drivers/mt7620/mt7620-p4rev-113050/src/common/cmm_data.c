...
// liudf added 20150403
// add mac table print function
void MacTableShow(PRTMP_ADAPTER pAd) {
	MAC_TABLE_ENTRY *pEntry = NULL;
	UINT32 MaxWcidNum = MAX_LEN_OF_MAC_TABLE;	
	int idx = 0;
	
	printk("apidx\tEntryType\tAddr\tHdrAddr1\tHdrAddr2\tHdrAddr3\n");
	NdisAcquireSpinLock(&pAd->MacTabLock);
	for(idx = 1; idx < MaxWcidNum; idx++) { // start from 1
		if (IS_ENTRY_NONE(&pAd->MacTab.Content[idx])) {
			continue;
		}	
		pEntry = &pAd->MacTab.Content[idx];
		printk("%d\t%d\t%02X:%02X:%02X:%02X:%02X:%02X\t%02X:%02X:%02X:%02X:%02X:%02X\t%02X:%02X:%02X:%02X:%02X:%02X\t%02X:%02X:%02X:%02X:%02X:%02X",
			pEntry->apidx, pEntry->EntryType, 
			PRINT_MAC(pEntry->Addr), PRINT_MAC(pEntry->HdrAddr1), 
			PRINT_MAC(pEntry->HdrAddr2), PRINT_MAC(pEntry->HdrAddr3));
	}
	NdisReleaseSpinLock(&pAd->MacTabLock); // liudf added	
}
	...
	for (i = FirstWcid; i< MaxWcidNum; i++)   /* skip entry#0 so that "entry index == AID" for fast lookup*/
	{
		...
		if (IS_ENTRY_NONE(&pAd->MacTab.Content[i]))
		{
			...
			if (CleanAll == TRUE)
			{
				...
				// liudf comment this line; 20140403
				//NdisZeroMemory(pEntry, sizeof(MAC_TABLE_ENTRY));
