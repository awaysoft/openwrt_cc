	...
	IN	PSTRING			mac_addr)
{
	...
	DBGPRINT(RT_DEBUG_OFF, ("--> EEPROMAddressNum = %d\n", pAd->EEPROMAddressNum ));

	/* Read MAC setting from EEPROM and record as permanent MAC address */
	DBGPRINT(RT_DEBUG_OFF, ("Initialize MAC Address from E2PROM \n"));
	
	// liudf 20150403
	// change get mac address position; from 0x02 to 0x2e
	RT28xx_EEPROM_READ16(pAd, 0x2e, Addr01);
	RT28xx_EEPROM_READ16(pAd, 0x30, Addr23);
	RT28xx_EEPROM_READ16(pAd, 0x32, Addr45);
