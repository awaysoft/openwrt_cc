...
#if	0
	// liudf comment the following code; 20150407
	/* allocate memory */
	os_alloc_mem(NULL, (UCHAR **)&pMacTab, sizeof(RT_802_11_MAC_TABLE));
	if (pMacTab == NULL)
	{
...
/*			pDst->TxPER = pEntry->SaveTxPER; */
	...
	sprintf(msg,"%s","\n");
	sprintf(msg+strlen(msg),"%-19s%-4s%-4s%-6s%-6s%-6s%-6s%-10s%-10s%-10s\n",
	...
	for (i=0; i<MAX_LEN_OF_MAC_TABLE; i++)
	{
...
#endif /* !DPA_S */
		{
			...
			sprintf(msg+strlen(msg),"%-6d", (int)pEntry->PsMode);
			sprintf(msg+strlen(msg),"%-6d", (int)pEntry->AuthState);
	...
	// liudf added 20150407
	// print mac table information
	wrq->u.data.length = strlen(msg);
