...
vif VIF =
{
    ...
    // liudf added 20150326
    // add support for apcli
    .apcli_enable	= {NULL, "apcli_enable", {0}, NULL, NULL},
    .apcli_ssid		= {NULL, "apcli_ssid", {0}, NULL, NULL},
    // liudf end
...
param CFG_ELEMENTS[] =
{
    ...
    */
    ...
    // liudf modified 20140325; add support for ap cli
    {"ApCliEnable", "apcli_enable", {0}, hooker, "0"},
    {"ApCliSsid", "apcli_ssid", {0}, hooker, NULL},
    {"ApCliBssid", NULL, {0}, NULL, NULL},
    {"ApCliAuthMode", "apcli_authmode", {0}, hooker, "OPEN"},
    {"ApCliEncrypType", "apcli_encryptype", {0}, hooker, "NONE"},
...
void hooker(FILE * fp, param * p, const char * devname)
{
    ...
    // support apcli; only open mode and only one apcli if support
    else if(0 == strcmp(p->dat_key, "ApCliEnable")) {
        FPRINT(fp, p, "%s", wifi_cfg[N].vifs[0].apcli_enable.value);
    }
    else if(0 == strcmp(p->dat_key, "ApCliSsid")) {
        FPRINT(fp, p, "%s", wifi_cfg[N].vifs[0].apcli_ssid.value);
    }
...
int main(int argc, char ** argv)
{
    ...
    {
    	printf("uci_load wireless failed\n");
        return NG;
    }
	...
	//printf("into debug mode\n");
		...
		// liudf debug
		//printf("debug: device %s profile %s\n", device, profile);
